...
def seat_economy(plane, economy_sold, name):
  ...
  while not (found_seat) and r_row < rows:
    '''
        r_row = random.randrange(0,rows)
        r_col = random.randrange(0,cols)
        if plane[r_row][r_col] == "win" or plane[r_row][r_col] == "avail":
            ...
            found_seat = True
        '''
    # search that random row for empty seats
    r_col = 0
    # number of seats needed for family
    needed_seats = economy_sold[name]  # 1-3
    '''
        starts on left most side, goes to the right
        needs to check if you hit the right wall w/o finding a seat
        needs to check right if seat is not found
        ...
        if avail:
          while fam > 0, check next
        else:
          r_col += 1
        
        '''
    # check for family sizing
    while r_col < cols - needed_seats:
      available = plane[r_row][r_col] == "win" or plane[r_row][r_col] == "avail"
      if available:
        if find_family_seats(plane, r_row, r_col, needed_seats):
          for i in range(0, needed_seats):
            plane[r_row][r_col + i] = name
          found_seat = True
          break
        else:
          r_col += 1
  ...
  # either found_seat == True or r_row == rows
  if found_seat == False:
    for i in range(economy_sold[name]):
      seat_individually(plane, economy_sold, name)

  return plane


# the old seat_economy. Oops.
def seat_individually(plane, economy_sold, name):
...
# called if plane[s_row][s_col] is available
def find_family_seats(plane, s_row, s_col, family_size):

  found_seats = 1

  while found_seats < family_size:
    # check the next col, +1 to found_seats
    s_col += 1
    available = plane[s_row][s_col] == "win" or plane[s_row][s_col] == "avail"
    if available:
      found_seats += 1
    else:
      return False
...
def fill_plane(plane):
  ...
  # look for largest family first
  family_size = max_family_size

  while family_size > 0:

    for name in economy_sold.keys():  # Robinson
      # if economy_sold[name] == max_family_size
      # then plan = seat_economy
      if economy_sold[name] == family_size:
        plane = seat_economy(plane, economy_sold, name)

    family_size -= 1

  # start with max family size, check each key value to see if matches family size
  # if matches, run seating alg, if not, skip
  # once max fam size checked, dec fam size - 1 and repeat till fam size = 0\

  # economy_sold[name] is the size of the family
